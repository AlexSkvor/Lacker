...
class TasksFragment : ToolbarFluxFragment<Wish, State>() {
     ...
     * * обработать
     *
     * Для пагинации используем "Опорный элемент", то есть передается id подзаказа / вызова и
     * указывается сколько элементов, которые старше опорного по времени создания вернуть
     *
     * Из деталей подзаказа можно посмотреть весь заказ
     */
